...
void DynamixelController::merge_array(int32_t *pos123, int32_t *pos456, int32_t *pos)
{
  ...
  for(int skip=0; skip < 2; skip++)
  {
    for( int i=0; i < 3; i++)
    {
      ...
      {
        pos[i+3*skip] = pos456[i];
        //ROS_INFO_STREAM(pos[i+3*skip]);
      }
    }
  }
}
...
bool DynamixelController::getPresentPosition(std::vector<std::string> dxl_name)
{
  ...
  int32_t get_position123[3];
  int32_t get_position456[3];
  
  uint8_t id_shoulder[3]= {1,2,3};
  uint8_t id_wrist[3] = {4,5,6};
  ...
  if (dxl_wb_->getProtocolVersion() == 2.0f)
  {
    ...
    {
      //merge position from PH motors (joints 1, 2, 3) 
      //with position from MX motors (joints 4, 5, 6)
      merge_array(get_position123, get_position456, get_position);
...
bool DynamixelController::turnTorque(bool off)
{
...
void DynamixelController::readCallback(const ros::TimerEvent&)
{
  ...
  int32_t get_current123[3];
  int32_t get_current456[3];
  int32_t get_velocity123[3];
  int32_t get_velocity456[3];
  int32_t get_position123[3];
  int32_t get_position456[3];
  ...
  if (is_moving_ == false)
  {
    ...
    if (dxl_wb_->getProtocolVersion() == 2.0f)
    {
      ...
      //merge current, velocity, position
      merge_array(get_current123, get_current456, get_current);
      merge_array(get_velocity123, get_velocity456, get_velocity);
      merge_array(get_position123, get_position456, get_position);
...
void DynamixelController::writeCallback(const ros::TimerEvent&)
{
  ...
  int32_t dynamixel_position123[3];
  int32_t dynamixel_position456[3];
  ...
  if (is_moving_ == true)
  {
    for (uint8_t index = 0; index < 3; index++)
      dynamixel_position123[index] = dxl_wb_->convertRadian2Value(id_shoulder[index], jnt_tra_msg_->points[point_cnt].positions.at(index));
    
    for (uint8_t index = 0; index < 3; index++)
      dynamixel_position456[index] = dxl_wb_->convertRadian2Value(id_wrist[index], jnt_tra_msg_->points[point_cnt].positions.at(index+3));

    result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION123, id_shoulder, 3, dynamixel_position123, 1, &log);
    if (result == false)
    {
      ROS_ERROR("%s", log);
    }
    
    result = dxl_wb_->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_POSITION456, id_wrist, 3, dynamixel_position456, 1, &log);
    if (result == false)
    {
      ROS_ERROR("%s", log);
    }
